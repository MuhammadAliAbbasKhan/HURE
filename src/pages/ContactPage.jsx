          ...
          <div className="flex flex-col lg:flex-row gap-12 items-center justify-between">
            ...
            <div className="w-full lg:w-1/2 flex justify-center items-center">
              <div className="relative w-80 h-80 sm:w-96 sm:h-96">
                ...
                >
                  {/* Social media icons positioned on the orbit */}
                  {socialMedia.map((social, index) => {
                    const position = getOrbitPosition(index, socialMedia.length, 140);
                    return (
                      <motion.a
                        key={social.name}
                        href={social.link || '#'}
                        ...
                        className={`absolute w-14 h-14 rounded-full ${social.color} flex items-center justify-center shadow-lg cursor-pointer z-20`}
                        style={{
                          left: `calc(50% + ${position.x}px - 1.75rem)`,
                          top: `calc(50% + ${position.y}px - 1.75rem)`
                        }}
                        whileHover={{
                          scale: 1.2,
                ...
                {socialMedia.map((social, index) => {
                  ...
                  return (
                    <motion.div
                      key={`tooltip-${social.name}`}
                      className="absolute z-30"
                      style={{
                        left: `calc(50% + ${position.x * 1.3}px - 2rem)`,
                        top: `calc(50% + ${position.y * 1.3}px - 1rem)`,
                      }}
                      initial={{ opacity: 0 }}
