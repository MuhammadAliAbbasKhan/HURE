// import React, { useState } from 'react';
// import ReactPlayer from 'react-player';
// import { FaTimes } from 'react-icons/fa';
// import '.././ProjectExplorer.scss';

// import scssVariables from '../../../styles/colors.module.scss';
// import websites from '../../../utils/Website/Website_Info.js';
// import android_projects from '../../../utils/WebdevData/Android_Data.js'; // Import your Android projects data

// const Android_Projects = () => {
//   const [fullscreenMedia, setFullscreenMedia] = useState(null);
//   const [mediaType, setMediaType] = useState(null);
//   const [hoveredCard, setHoveredCard] = useState(null);
//   const [popupMessage, setPopupMessage] = useState(null);

//   const handleMediaClick = (mediaUrl, type) => {
//     if (!mediaUrl) {
//       setPopupMessage("This project preview is not currently available. Please check back later!");
//       return;
//     }
//     setFullscreenMedia(mediaUrl);
//     setMediaType(type);
//   };

//   return (
//     <div 
//       className="project-explorer py-12 mt-48 px-4 sm:px-6 lg:px-8" 
//       style={{ backgroundColor: scssVariables?.colorBgSecondary || '#f8f9fa' }}
//     >
//       <div className="max-w-7xl mx-auto">
//         <div className="text-center mb-12">
//           <h2 
//             className="text-3xl md:text-4xl italic font-bold mb-4" 
//             style={{ 
//               color: scssVariables?.colorBlack || '#212529',
//               fontFamily: scssVariables?.headings || 'sans-serif'
//             }}
//           >
//             Android Development Projects
//           </h2>
//           <div 
//             className="w-24 h-1 mx-auto" 
//             style={{ backgroundColor: scssVariables?.green || '#28a745' }}
//           />
//         </div>

//         {fullscreenMedia && (
//           <div 
//             className="fixed inset-0 z-50 flex items-center justify-center p-4" 
//             style={{ 
//               backgroundColor: scssVariables?.colorBlack || '#000',
//               opacity: 0.95
//             }}
//           >
//             <button 
//               onClick={() => {
//                 setFullscreenMedia(null);
//                 setMediaType(null);
//               }}
//               className="absolute top-6 right-6 text-3xl transition-colors"
//               style={{ 
//                 color: scssVariables?.colorWhite || '#fff',
//                 ':hover': {
//                   color: scssVariables?.green || '#28a745'
//                 }
//               }}
//             >
//               &times;
//             </button>
//             <div className="w-full max-w-6xl">
//               {mediaType === 'video' ? (
//                 <div className="aspect-video">
//                   <ReactPlayer
//                     url={fullscreenMedia}
//                     controls
//                     width="100%"
//                     height="100%"
//                     playing
//                   />
//                 </div>
//               ) : (
//                 <div className="flex justify-center">
//                   <img 
//                     src={fullscreenMedia} 
//                     alt="Project preview" 
//                     className="max-h-[90vh] max-w-full object-contain"
//                     onError={(e) => {
//                       e.target.onerror = null; 
//                       e.target.src = websites[0]?.logo || '/default-project.png';
//                     }}
//                   />
//                 </div>
//               )}
//             </div>
//           </div>
//         )}

//         {popupMessage && (
//           <div 
//             className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-70"
//             onClick={() => setPopupMessage(null)}
//           >
//             <div 
//               className="relative max-w-md w-full p-6 rounded-lg shadow-lg"
//               style={{
//                 backgroundColor: scssVariables?.colorWhite || '#fff',
//                 color: scssVariables?.colorGray800 || '#343a40',
//                 fontFamily: scssVariables?.text || 'sans-serif'
//               }}
//               onClick={(e) => e.stopPropagation()}
//             >
//               <button 
//                 onClick={() => setPopupMessage(null)}
//                 className="absolute top-4 right-4 transition-colors"
//                 style={{ 
//                   color: scssVariables?.colorGray600 || '#6c757d',
//                   ':hover': {
//                     color: scssVariables?.colorGray800 || '#343a40'
//                   }
//                 }}
//               >
//                 <FaTimes size={20} />
//               </button>
//               <div className="text-center">
//                 <h3 
//                   className="text-xl font-bold mb-3"
//                   style={{
//                     color: scssVariables?.colorGray800 || '#343a40',
//                     fontFamily: scssVariables?.subheadings || 'sans-serif'
//                   }}
//                 >
//                   Notice
//                 </h3>
//                 <p className="mb-4">{popupMessage}</p>
//                 <button
//                   onClick={() => setPopupMessage(null)}
//                   className="px-6 py-2 rounded-md transition-colors"
//                   style={{
//                     backgroundColor: scssVariables?.green || '#28a745',
//                     color: scssVariables?.colorWhite || '#fff',
//                     ':hover': {
//                       backgroundColor: scssVariables?.colorSuccess || '#218838'
//                     }
//                   }}
//                 >
//                   Close
//                 </button>
//               </div>
//             </div>
//           </div>
//         )}

//         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
//           {android_projects?.map((project) => {
//             const hasVideo = !!project?.videoUrl;
//             const hasImage = !!project?.imageUrl;
//             const previewMedia = hasVideo ? project.videoUrl : (hasImage ? project.imageUrl : null);
//             const mediaType = hasVideo ? 'video' : (hasImage ? 'image' : null);

//             return (
//               <div 
//                 key={project?.id || Math.random()} 
//                 className="aspect-[4/3] perspective-1000"
//                 onMouseEnter={() => setHoveredCard(project?.id)}
//                 onMouseLeave={() => setHoveredCard(null)}
//               >
//                 <div className={`relative w-full h-full transition-all duration-500 transform-style-preserve-3d ${hoveredCard === project?.id ? 'rotate-y-180' : ''}`}>
//                   <div 
//                     className="absolute inset-0 backface-hidden rounded-xl p-6 shadow-md flex flex-col justify-center"
//                     style={{
//                       backgroundColor: scssVariables?.colorWhite || '#fff',
//                       borderColor: scssVariables?.colorBorder || '#dee2e6',
//                       boxShadow: scssVariables?.shadowMd || '0 4px 6px -1px rgba(0, 0, 0, 0.1)',
//                       borderWidth: '1px'
//                     }}
//                   >
//                     <div className="text-center">
//                       <div className="flex justify-center mb-4">
//                         <div 
//                           className="w-20 h-20 rounded-full overflow-hidden border-2"
//                           style={{
//                             borderColor: scssVariables?.green || '#28a745'
//                           }}
//                         >
//                           <img 
//                             src={project?.iconUrl || websites[0]?.logo || '/default-logo.png'} 
//                             alt={project?.title || 'Project'}
//                             className="w-full h-full object-cover"
//                             onError={(e) => {
//                               e.target.onerror = null; 
//                               e.target.src = '/default-logo.png';
//                             }}
//                           />
//                         </div>
//                       </div>
                      
//                       <h3 
//                         className="text-xl font-bold mb-3"
//                         style={{
//                           color: scssVariables?.colorGray800 || '#343a40',
//                           fontFamily: scssVariables?.subheadings || 'sans-serif'
//                         }}
//                       >
//                         {project?.title || 'Android Project'}
//                       </h3>
//                       <p 
//                         className="mb-4 px-2"
//                         style={{
//                           color: scssVariables?.colorGray600 || '#6c757d',
//                           fontFamily: scssVariables?.text || 'sans-serif'
//                         }}
//                       >
//                         {project?.description || 'Description not available'}
//                       </p>
//                       <div 
//                         className="w-16 h-1 mx-auto"
//                         style={{ backgroundColor: scssVariables?.green || '#28a745' }}
//                       ></div>
//                     </div>
//                   </div>

//                   <div 
//                     className="absolute inset-0 backface-hidden rounded-xl overflow-hidden transform rotate-y-180"
//                     style={{ backgroundColor: scssVariables?.green || '#28a745' }}
//                   >
//                     <div className="relative w-full h-full">
//                       {mediaType === 'video' ? (
//                         <ReactPlayer
//                           url={previewMedia}
//                           width="100%"
//                           height="100%"
//                           muted
//                           loop
//                           playing={hoveredCard === project?.id}
//                           playsinline
//                           className="project-video"
//                         />
//                       ) : mediaType === 'image' ? (
//                         <img 
//                           src={previewMedia} 
//                           alt={project?.title || 'Project preview'}
//                           className="w-full h-full object-cover"
//                           onError={(e) => {
//                             e.target.onerror = null; 
//                             e.target.src = websites[0]?.logo || '/default-project.png';
//                           }}
//                         />
//                       ) : (
//                         <div 
//                           className="w-full h-full flex items-center justify-center"
//                           style={{ backgroundColor: scssVariables?.green || '#28a745' }}
//                         >
//                           <p 
//                             className="text-center p-4"
//                             style={{
//                               color: scssVariables?.colorWhite || '#fff',
//                               fontFamily: scssVariables?.text || 'sans-serif'
//                             }}
//                           >
//                             Preview not available
//                           </p>
//                         </div>
//                       )}
                      
//                       <div 
//                         className="absolute inset-0 flex items-center justify-center gap-4"
//                         style={{ backgroundColor: 'rgba(0, 0, 0, 0.4)' }}
//                       >
//                         {mediaType && (
//                           <button
//                             onClick={(e) => {
//                               e.stopPropagation();
//                               handleMediaClick(previewMedia, mediaType);
//                             }}
//                             className="w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-colors"
//                             title={mediaType === 'video' ? "View Demo" : "View Preview"}
//                             style={{
//                               backgroundColor: scssVariables?.green || '#28a745',
//                               color: scssVariables?.colorWhite || '#fff',
//                               ':hover': {
//                                 backgroundColor: scssVariables?.colorSuccess || '#218838'
//                               }
//                             }}
//                           >
//                             {mediaType === 'video' ? (
//                               <span className="ml-1 text-xl">▶</span>
//                             ) : (
//                               <span className="text-xl">🔍</span>
//                             )}
//                           </button>
//                         )}
//                       </div>
//                     </div>
//                   </div>
//                 </div>
//               </div>
//             );
//           })}
//         </div>
//       </div>
//     </div>
//   );
// };

// export default Android_Projects;